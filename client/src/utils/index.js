...
export const apiRequest = async ({ url, token, data, method }) => {
	try {
		...
		return typeof newdata === "string" ? JSON.parse(newdata) : newdata;
	} catch (error) {
		console.log(error);
		return error.response?.data || { status: "failed", message: error.message };
	}
};

export const apiFormRequest = async ({ url, token, data, method }) => {
	try {
		const result = await API(url, {
			headers: {
				Authorization: token ? `Bearer ${token}` : "",
